...
3. JSON 必须包含以下字段：
   ...
   - "code_questions": 字符串数组，输出 5 个更贴代码本身、用于后续澄清专利写法的问题。
   - "claim_mainline": 字符串数组，列出 3-5 条后续权利要求的主线组织思路。
4. 每个问题都要具体，优先围绕输入定义、模块边界、融合/时序/损失/训练策略是否进入权利要求、以及技术效果证据。
5. 不要把“有某个类/函数/常量”本身当作专利点，除非它承载了明确的技术机制。
...
def _fallback_questions_from_extraction(
    ...
    templates = [
        "围绕“{anchor}”，你希望权利要求写成明确步骤/模块组合，还是保留更宽的功能性表述？",
        "“{anchor}”对应的输入输出边界要写到多细？哪些参数、时序关系或中间结果必须限定？",
        "如果“{anchor}”要作为核心保护点，哪些替代实现也应落入保护范围，哪些实现细节只放实施例？",
        "“{anchor}”带来的技术效果准备如何证明？是否有对比实验、指标提升或适用工况可用于支撑？",
        "“{anchor}”与训练策略、损失函数或阈值配置之间，哪些关系是发明必要条件，哪些只是调参结果？",
    ]
    ...
    generic_questions = [
        "代码中最想保护的核心技术机制到底是哪一段流程，是否需要明确限定其前后依赖关系？",
        "输入信号、特征张量和最终输出结果之间，哪些边界条件必须写入权利要求，哪些可以保持开放？",
        "哪些模块属于核心发明步骤，哪些训练或部署细节只适合放在实施例而不进入权利要求？",
        "如果要覆盖替代实现，目前代码里的哪几个实现细节应抽象成更宽的功能性表述？",
        "现有实验中最能支撑该发明效果的指标、场景或对比基线分别是什么？",
    ]
